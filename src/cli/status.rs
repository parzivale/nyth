...
/// A path in `upper`: copied up on write, or created fresh.
...
pub enum PendingChange {
    /// Backed by a real source file in the dotfiles repo (`home.file.<name>.source`)
    RepoBacked { relative_path: PathBuf },
    /// Rendered by a `programs.*` module from Nix options (`home.file.<name>.text`)
    Generated { relative_path: PathBuf },
    /// Not managed by Home Manager at all
    Untracked { relative_path: PathBuf },
}

/// `root` is a repo that mirrors $HOME 1:1 for the repo-backed subset.
...
impl DotfilesRepo {
    ...
    /// Pure lookup: which of the three states `entry`'s path falls into
...
/// argv-parsed inputs to `nyth status`/`nyth commit`
...
fn parse_watched_path(
    ...
    // Folded into the message, not `#[context]`: this is argv the user typed
...
/// The only I/O in `status`: walks `upper` once, hands off to the pure diff above.
pub fn nyth_status(
    paths: &NythPaths,
    repo: &DotfilesRepo,
